...
All prices in **dollars per kWh**, not cents, not $/MWh. Tariff documents publish in all three. Converting is the single most common extraction error — see "Units" below. The one exception is the gas utility file, whose commodity prices are **dollars per therm** — see "Gas utility file" below.
...
## Gas utility file — `rates/<utility>-gas.json`

Residential natural gas (SDG&E Schedule GR). The simplest rate file the calculator carries, and its shape is dictated by what the schedule actually publishes — **do not model it on the electric file**:

- **No TOU, no NEM, no CCA generation choice** — no plans array, no `day_types`, no `nonbypassable_charges`, no hour blocks.
- **No climate zones.** The baseline allowance is territory-wide.
- **No daily service charge** — only a per-day **minimum-bill floor**.
- Prices are **`$/therm`**.

A bill is `(therms up to the baseline allowance × baseline rate) + (therms above it × non-baseline rate)`, floored at `minimum_bill.per_day × days`.
...
```json
{
  ...
  "source_url": "https://…",
  "verified_against": "SDG&E Schedule GR, 'GR' column, Sheet 1 (rates) and Sheet 2 (baseline), Advice Ltr. …, effective …",

  "minimum_bill": { "per_day": 0.13151, "per_day_care": 0.10521 },

  "baseline_periods": {
    "summer":          { "months": [5, 6, 7, 8, 9, 10], "allowance_therms_per_day": 0.359 },
    "winter_on_peak":  { "months": [12, 1, 2],          "allowance_therms_per_day": 1.233 },
    "winter_off_peak": { "months": [11, 3, 4],          "allowance_therms_per_day": 0.692 }
  },

  "rates_per_therm": { "baseline": 2.29854, "non_baseline": 2.66686 }
}
```

- **`baseline_periods`** is a set of named periods that **partition all twelve months exactly once** (the validator errors on a gap or overlap). SDG&E gas uses three — summer, winter-on-peak, winter-off-peak — not the electric file's two seasons. Each carries its own therms/day allowance.
- **`rates_per_therm`** is two flat prices, not a season tree. They do not vary by month; only the allowance does. Each is the schedule's *total* (Procurement + Transmission) for the standard residential **GR** column. `non_baseline` must be ≥ `baseline` (the validator rejects a swap). The validator warns outside roughly **$0.30–$5.00/therm** — a value far outside that is usually a `$/MMBtu` (1 therm ≈ 0.1 MMBtu) or `¢/therm` figure pasted without converting.

Read off the **GR** column only. Sheet 1 also prints **GR-C** (cross-over, for customers over 50,000 therms/yr) and **GTC/GTCA** (transportation-only, no procurement) — neither is the standard individually-metered residential rate. The Procurement Charge bundles Schedule GPC and resets monthly, so record the effective-date snapshot and note it in `notes`. Small per-therm surcharges (G-PUC, G-PPPS, franchise fee) and the semi-annual California Climate Credit sit in separate schedules and are not modeled — say so in `notes`.
...
Generated by `scripts/reindex.mjs`. Never hand-edited. Recognises five file types: `utility`, `utility_gas`, `generation`, `export_prices`, `cities`.

## Past revisions — `rates/history/`

A billing period can span a rate change, so the calculator needs the revision in force on each day, not just the current one. Superseded revisions go in `rates/history/` and get their own generated manifest.

- **Identical schema.** A history file is a previous revision, not a new kind of document. Promoting one to current is a move, not an edit.
- **One revision per effective date**, per provider + type + product + `rate_group`. `reindex.mjs` errors on a collision, because resolution would otherwise depend on load order.
- **Every archived `effective_date` must be older** than the current file's. A newer one means the two were swapped, and the archive would quietly reprice the present.
- **Partial files are allowed and expected.** A history file may carry only the schedules that have been extracted. Plans absent from it fall back to the current revision *with a warning* — never silently. Do not read a missing plan as "unchanged".
- Record in `_notes` which numbers were read off the archived document and which were carried over from the newer revision unverified. The TOU hour windows are the usual carry-over: a Total Rates Table prints prices by period name, not the clock hours those periods span.

Find and fetch past revisions with `scripts/fetch-rate-history.mjs`. SDG&E's dated Total Rates Table PDFs stay at their original URLs; the tariff API has no history endpoint. The cadence is irregular — probe for dates, do not generate them from a rule.
...
### `months` — a window that applies to part of a season

Optional. A block may carry `"months": [3, 4]`, restricting it to those calendar months. A month-scoped block **wins over** the unscoped block it overlaps, which is how a narrow rule carves a window out of a broader one:

```json
{ "start_hour": 6,  "end_hour": 16, "price_per_kwh": 0.32302 },
{ "start_hour": 10, "end_hour": 14, "months": [3, 4], "price_per_kwh": 0.04705 }
```

Tiling is then checked **per month, after precedence** — every hour of every month must resolve to exactly one price. Narrowing a block to `[3, 4]` without leaving something spanning the other ten months is a gap, and the validator rejects it.

**Reach for a revision first.** A window that applies to part of a season looks identical, in one bill, to a window that was changed by a tariff revision. SDG&E's winter 10am–2pm super-off-peak window read "in March and April" on a Feb–Mar 2026 bill and read as unrestricted on the next one — it was a revision, not a season rule, and belongs in `rates/history/`. No shipped rate file uses `months` today. Use it only where the *current* tariff sheet states the restriction.
